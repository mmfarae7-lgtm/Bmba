...
async function getUserFromRequest(req) {
  ...
  if (token) {
    ...
    if (decoded) {
      const user = await db.prepare('SELECT id, name, phone, email, points, role, blocked, avatar, phone_code, country, gender, birth_date, bombs, invite_code, invited_by, notifications, google_id, facebook_id FROM users WHERE id = ?').get(decoded.id);

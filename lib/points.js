...
async function calculatePointsForMatch(matchId) {
  ...
  for (const pred of predictions) {
    ...
    if (pts > 0) {
      ...
      await sendCorrectPredictionPush(pred.user_id, match, pts, exact);
      let reasons = [];
      if (exact) {
...
// إشعار فوري (Web Push) عند توقع صحيح — إشعارات تصل حتى مع إغلاق التطبيق
async function sendCorrectPredictionPush(userId, match, pts, exact) {
  try {
    const { sendPushToUser } = require('./push');
    const teams = (match.home_team || '') + ' ' + (match.home_score ?? '') + '-' + (match.away_score ?? '') + ' ' + (match.away_team || '');
    const body = exact
      ? 'توقعك الدقيق صحيح! ' + teams + ' — حصلت +' + pts + ' بمبا'
      : 'توقعت فوز/تعادل ' + (match.home_team || '') + ' ضد ' + (match.away_team || '') + ' وحصلت +' + pts + ' بمبا';
    await sendPushToUser(userId, '🎯 توقع صحيح!', body, '/');
  } catch {}
}

module.exports = { calculatePointsForMatch, exactPoints, sendCorrectPredictionPush };

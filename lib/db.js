...
async function init() {
  ...
  for (const stmt of [
    ...
    `CREATE TABLE IF NOT EXISTS store_products (
      ...
      active INTEGER DEFAULT 1,
      created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
      FOREIGN KEY (merchant_id) REFERENCES store_merchants(id) ON DELETE SET NULL
    );`,
    `CREATE TABLE IF NOT EXISTS store_vouchers (
      ...
      cost_bombs INTEGER NOT NULL DEFAULT 0,
      active INTEGER DEFAULT 1,
      created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
      FOREIGN KEY (merchant_id) REFERENCES store_merchants(id) ON DELETE SET NULL
    );`,
    `CREATE TABLE IF NOT EXISTS user_vouchers (
      ...
      used_at DATETIME,
      FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE,
      FOREIGN KEY (voucher_id) REFERENCES store_vouchers(id) ON DELETE CASCADE
    );`,
    ...
    `CREATE INDEX IF NOT EXISTS idx_otps_phone ON otps(phone, purpose, used);`,
  ...
  await addColumn('users', 'facebook_id', 'TEXT');
  await client.execute(`CREATE UNIQUE INDEX IF NOT EXISTS idx_users_google ON users(google_id) WHERE google_id IS NOT NULL`);
  await client.execute(`CREATE UNIQUE INDEX IF NOT EXISTS idx_users_facebook ON users(facebook_id) WHERE facebook_id IS NOT NULL`);
  ...
  // ——— متجر بمبا: الشركاء والمنتجات والقسائم ———
  const merchFlag = (await client.execute("SELECT value FROM settings WHERE key = 'bomba_store_seed_v1'")).rows[0]?.value;
  if (merchFlag !== '1') {
    const merchants = [
      ['اديداس النسيم', 'shop', 'ملابس وأدوات رياضية', 'فرع متخصص في الملابس والأحذية الرياضية الأصلية', '🏬', 'حي النسيم، الرياض'],
      ['نايك الرياض', 'shop', 'ملابس رياضية', 'أحدث تشكيلات الملابس والأحذية الرياضية', '👟', 'الرياض — حي العليا'],
      ['رياض جيم', 'shop', 'أدوات رياضية', 'أجهزة ومعدات الجيم واللياقة', '🏋️', 'طريق الملك فهد'],
      ['مطعم كابتن برجر', 'restaurant', 'مطعم', 'برجر وحمص ومشويات للملاعب واللقاءات', '🍔', 'الرياض — الملقا'],
      ['كافيه النخبة', 'restaurant', 'كافيه', 'مشروبات وسناكات بعد المباريات', '☕', 'الخبر — الكورنيش'],
      ['متجر الرشاقة', 'store', 'متجر صحي', 'مكملات وأغذية صحية للرياضيين', '🥗', 'جدة — حي الروضة'],
    ];
    for (const [name, type, category, desc, logo, location] of merchants) {
      await client.execute({
        sql: 'INSERT INTO store_merchants (name, type, category, description, logo, location) VALUES (?, ?, ?, ?, ?, ?)',
        args: [name, type, category, desc, logo, location],
      });
    }
    const mids = (await client.execute('SELECT id FROM store_merchants ORDER BY id')).rows.map((r) => r.id);
    if (mids.length >= 2) {
      const products = [
        [mids[0], 'قميص منتخبنا 2026', 'اديداس', 'قميص أصلي عالي الجودة', '👕', 249],
        [mids[0], 'حذاء الجري ألترابوست', 'اديداس', 'حذاء خفيف ومريح', '👟', 549],
        [mids[1], 'جاكيت الشتاء الرياضي', 'نايك', 'جاكيت دافئ مع تقنية Dri-FIT', '🧥', 399],
        [mids[1], 'شورت الجيم', 'نايك', 'شورت مطاطي مريح', '🩳', 129],
        [mids[2], 'دمبل قابلة للتعديل 20كجم', 'رياض جيم', 'ثنائية بتعديل سريع', '🏋️', 420],
        [mids[2], 'حبل مقاومة مطاطي', 'رياض جيم', 'للتمارين المنزلية', '🤸', 85],
      ];
      for (const [mid, name, brand, desc, img, price] of products) {
        await client.execute({
          sql: 'INSERT INTO store_products (merchant_id, name, brand, description, image, price_sar) VALUES (?, ?, ?, ?, ?, ?)',
          args: [mid, name, brand, desc, img, price],
        });
      }
    }
    if (mids.length > 0) {
      const vouchers = [
        [mids[0], 'خصم 20%', 'خصم 20% على الملابس الرياضية', 'discount', 20, 300],
        [mids[1], 'خصم 50%', 'خصم 50% على تشكيلة مختارة', 'discount', 50, 800],
        [mids[2], 'خصم 70%', 'خصم 70% على الأدوات المنزلية', 'discount', 70, 1200],
        [mids[3], 'وجبة كابتن برجر', 'وجبة برجر + مشروب مجاناً', 'item', 0, 600],
        [mids[4], 'كافيه النخبة 2+1', 'قسمية قهوتين والثالثة علينا', 'item', 0, 400],
        [mids[5], 'شتلة المكملات', 'كوبون شراء مكملات بقيمة 100 ريال', 'item', 0, 700],
      ];
      for (const [mid, title, desc, type, discount, cost] of vouchers) {
        await client.execute({
          sql: 'INSERT INTO store_vouchers (merchant_id, title, description, type, discount, cost_bombs) VALUES (?, ?, ?, ?, ?, ?)',
          args: [mid, title, desc, type, discount, cost],
        });
      }
    }
    await client.execute({
      sql: "INSERT INTO settings (key, value) VALUES ('bomba_store_seed_v1', '1') ON CONFLICT(key) DO UPDATE SET value = excluded.value",
    });
  }
    // سوق المدرب: أكبر قاعدة بيانات ممكنة من نجوم العالم
  // (الدوريات الخمسة الكبرى + دوري روشن السعودي، من كل المراكز)
  // الترقيم (v3) يضمن إضافة اللاعبين الجدد أيضاً لقواعد البيانات القائمة (الإنتاج والمحلي)
  const { PLAYERS } = require('./players-data');
  const seedFlag = (await client.execute("SELECT value FROM settings WHERE key = 'players_seed_v3'")).rows[0]?.value;
  if (seedFlag !== '1') {
    for (const [name, club, position, league, market_value_m] of PLAYERS) {
      const exists = await client.execute({
        sql: 'SELECT id FROM players WHERE name = ? AND club = ?',
        args: [name, club],
      });
      if (exists.rows.length === 0) {
        await client.execute({
          sql: `INSERT INTO players (name, club, position, league, market_value_m, price_bombs) VALUES (?, ?, ?, ?, ?, ?)`,
          args: [name, club, position, league, market_value_m, market_value_m * 10],
        });
      }
    }
    await client.execute({
      sql: "INSERT INTO settings (key, value) VALUES ('players_seed_v3', '1') ON CONFLICT(key) DO UPDATE SET value = excluded.value",
    });
  }

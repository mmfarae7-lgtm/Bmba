...
- ملاحظة: قبل إضافة رفع الشعارات، كانت شعارات «الشركاء» (🏬) رموزاً تعبيرية فقط.

## 🏬 رفع شعارات الشركاء في المتجر — منفَّذ

طلب المستخدم: إضافة رفع شعار الشريك في نموذج «إضافة شريك» في متجر بمبا، بنفس
طريقة رفع صور المنتجات.

- **نموذج إضافة/تعديل الشريك** في لوحة الأدمن `/admin` → «🛍️ متجر بمبا» →
  «🏬 الشركاء»: زر «📷 رفع شعار الشريك» يختار صورة من الجهاز وتُضغط تلقائياً
  (`compressImage` — أقصى 720px وجودة 82%) مع معاينة فورية، أو لصق رابط شعار
  مباشر `https://…` أو رمز تعبيري.
- **التخزين:** Data URL (base64) داخل حقل `store_merchants.logo` — مع إصلاح
  `addMerchant` ومسار PUT حتى لا يُقتطع الـ Data URL: القيمة تُحفظ كاملة إذا بدأت
  بـ `data:` / `https?:` / `/`، وإلا (رمز تعبيري) تُقتصر كالسابق على 8 محارف.
- **التعديل:** زر «✏️ تعديل» لكل شريك (PUT
  `?entity=merchant&id=`) — لاستبدال شعارات الشركاء الستة الافتراضية دون حذفهم.
- **العرض:** صفحة `/store` (تبويب «🏬 الشركاء») تعرض `<img class="sp-img">` داخل
  إطار مربع عندما يكون الشعار رابطاً/Data URL، وتعود للرمز التعبيري «🏪» كـ fallback.
  جدول الأدمن يعرض الشعار مصغّراً `.adm-img`. الأنماط قائمة من قسم المنتجات
  (`.upload-btn`/`.upload-preview`/`.adm-img`/`.sp-img-wrap`/`.sp-img`).
- **اختبار E2E محلي:** رفع شعار canvas حقيقي عبر PUT على شريك موجود ← ظهوره في
  `/store` كصورة فعلية في بطاقة الشريك ← نموذج الأدمن (زر الرفع، 6 أزرار تعديل،
  مصغّر في الجدول) ← فتح وضع تعديل نفس الشريك ومعاينة الشعار المرفوع ← التراجع
  للشعار الرمزي «🏬» — كل الفحوصات نجحت قبل الرفع.
